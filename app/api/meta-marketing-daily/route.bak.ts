                          ...
                          try {
                            ...
                            for (const ad of ads) {
                              try {
                                ...
                                // Apply the same multi-step approach for ads
                                console.log(
                                  `Storing ad ${ad.id} to meta_ads (minimal data)...`
                                );

                                // Step 1: Insert minimal ad data first
                                const minimalAdRecord = {
                                  ad_id: ad.id,
                                  account_id: accountId,
                                  ad_set_id: adSet.id,
                                  campaign_id: campaign.id,
                                ...
                                try {
                                  ...
                                  console.log(
                                    `Checking if ad ${ad.id} already exists in database...`
                                  );
                                  ...
                                  if (existingAd && existingAd.length > 0) {
                                    // Ad exists, use update instead of upsert to avoid foreign key issues
                                    console.log(
                                      `Ad ${ad.id} found in database. Using update approach...`
                                    );

                                    const { error: updateError } =
                                      await supabase
                                        .from("meta_ads")
                                        .update(minimalAdRecord)
                                        .eq("ad_id", ad.id);

                                    if (updateError) {
                                      console.error(
                                        `Error updating data for ad ${ad.id}:`,
                                        updateError
                                      );
                                      console.error(
                                        `Error details:`,
                                      ...
                                      console.log(
                                        `Successfully updated data for ad ${ad.id}`
                                      );
                                    }
                                  } else {
                                    // Ad doesn't exist, use upsert with careful error handling
                                    console.log(
                                      `Ad ${ad.id} not found in database. Using insert approach...`
                                    );

                                    const { error: minAdError } = await supabase
                                      .from("meta_ads")
                                      .upsert([minimalAdRecord], {
                                        onConflict: "ad_id",
                                        ignoreDuplicates: false,
                                      });
                                    ...
                                    if (minAdError) {
                                      console.error(
                                        `Error storing minimal data for ad ${ad.id}:`,
                                        minAdError
                                      );
                                      console.error(
                                        `Error details:`,
                                        JSON.stringify(minAdError, null, 2)
                                      );
                                    } else {
                                      console.log(
                                      ...
                                      // Step 2: Update with full ad data after successful minimal insert
                                      try {
                                        const fullAdRecord = {
                                          ...
                                          conversion_specs: ad.conversion_specs
                                            ? JSON.stringify(ad.conversion_specs)
                                            : null,
                                          preview_url:
                                          ...
                                          configured_status:
                                            ad.configured_status || null,
                                          effective_status:
                                            ad.effective_status || null,
                                          issues_info: Array.isArray(
                                            ad.issues_info
                                          )
                                          ...
                                          engagement_audience:
                                            ad.engagement_audience || null,
                                          object_story_spec: ad.object_story_spec
                                            ? JSON.stringify(ad.object_story_spec)
                                            : null,
                                          recommendations: Array.isArray(
                                            ad.recommendations
                                          )
                                            ? JSON.stringify(ad.recommendations)
                                            : JSON.stringify([]),
                                          tracking_and_conversion_specs:
                                            ad.tracking_and_conversion_specs
                                              ? JSON.stringify(
                                                  ad.tracking_and_conversion_specs
                                                )
                                              : null,
                                        ...
                                        // Update the ad with the full data
                                        ...
                                        if (fullUpdateError) {
                                          console.error(
                                            `Error updating full data for ad ${ad.id}:`,
                                            fullUpdateError
                                          );
                                        } else {
                                          console.log(
                                            `Successfully updated full data for ad ${ad.id}`
                                          );
                                        }
                                      } catch (fullUpdateError) {
                                        console.error(
                                          `Exception updating full data for ad ${ad.id}:`,
                                          fullUpdateError
                                        );
                                      }
                                    }
