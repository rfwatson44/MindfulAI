...
export default function ChartTab({
  ...
  if (selectedRange.type === "column") {
    ...
    selectedRange.values.forEach((v: any) => {
      ...
      adMap[v.adName][v.metricName || v.metricId || "Metric"] =
    ...
    if (colOrder.length === 2) {
      ...
      chartData = Object.values(adMap).map((row: any) => {
        const ratio =
          row[denominator] !== 0 && row[denominator] != null
            ? row[numerator] / row[denominator]
            : null;
        return {
          name: row.name,
          [`${numerator} / ${denominator}`]:
            ratio && isFinite(ratio) ? Math.round(ratio * 1000) / 1000 : null, // keep more precision for percent formatting
        };
      });
    ...
    selectedRange.values.forEach((v: any) => {
      ...
      if (!metricMap[v.metricName])
        metricMap[v.metricName] = { name: v.metricName };
      metricMap[v.metricName][v.adName || v.adId || "Ad"] =
        Math.round(Number(v.value) * 10) / 10;
    ...
    chartData = [
      {
        ...
        value:
          typeof selectedRange.value === "number"
            ? Math.round(selectedRange.value * 10) / 10
            : Math.round(Number(selectedRange.value) * 10) / 10,
  ...
  chartData = chartData.filter((d) =>
    Object.keys(d).some(
      (k) => k !== "name" && typeof d[k] === "number" && !isNaN(d[k])
    )
  );
  ...
  if (sortType.startsWith("value")) {
    ...
    chartData.sort((a, b) => {
      const aVal = Object.keys(a).find((k) => k !== "name")
        ? a[Object.keys(a).find((k) => k !== "name")!]
        : 0;
      const bVal = Object.keys(b).find((k) => k !== "name")
        ? b[Object.keys(b).find((k) => k !== "name")!]
        : 0;
